...
(function (global, factory) {
    ...
    function copyObject(object) {
        ...
        forEach(Object.keys(object), function (key) { return out[key] = object[key]; });
        return out;
    }
    var WIN = window;
    var DOM = document;
            ...
            var $OnArrayChanges = function (detail) {
                ...
                switch (method) {
                    ...
                    case 'splice': { // Indexed removal handler
                        ...
                        forEach(removedItems, function (item) { return $RemoveEl(getRootElement(item.el)); });
                        expObj = expObj || $ExpressionBuilder(trim(ifNullReturn(node.nodeValue, '')));
                        var leftHandParts = expObj.leftHandParts;
                        var indexOrValue = leftHandParts[1] || '_index_or_value';
                    ...
                    case 'unshift': { // Addition handler
                        ...
                        var element = listedItemsHandler[0].el;
    ...
    var Compiler = /** @class */ (function (_super) {
        ...
        Compiler.prototype.compile = function (options) {
            ...
            var routing = this.serviceProvider.get('Routing');
            if (!rootElement)
    ...
    var Component = /** @class */ (function (_super) {
        ...
        function Component(optionsOrPath) {
            ...
            Prop.set(_this, 'template', {
                get: function () { return template.value; },
                set: function (v) { return template.value = v; }
            });
            return _this;
        }
        ...
        Component.prototype.destroy = function () {
            ...
            this.events = [];
            var components = ServiceProvider.get(this.bouer, 'ComponentHandler')
                .activeComponents;
    ...
    var ViewChild = /** @class */ (function () {
        ...
        ViewChild.viewBy = function (bouerInstance, expression) {
            // Retrieving the active component
            var activeComponents = ServiceProvider.get(bouerInstance, 'ComponentHandler')
                .activeComponents;
            // Applying filter to the find the component
            return where(activeComponents, expression);
        };
        ViewChild.viewById = function (bouerInstance, componentId) {
            // Retrieving the active component
            var activeComponents = ServiceProvider.get(bouerInstance, 'ComponentHandler')
                .activeComponents;
            // Applying filter to the find the component
            return where(activeComponents, function (c) { return (c.el && getRootElement(c.el).id == componentId); });
        };
        ViewChild.viewByName = function (bouerInstance, componentName) {
            // Retrieving the active component
            var activeComponents = ServiceProvider.get(bouerInstance, 'ComponentHandler')
                .activeComponents;
            // Applying filter to the find the component
            return where(activeComponents, function (c) { return c.name.toLowerCase() == (componentName || '').toLowerCase(); });
        };
        return ViewChild;
    }());
    ...
    var Bouer = /** @class */ (function (_super) {
        ...
        function Bouer(selector, options) {
            ...
            _this_1.$components = {
                ...
                viewBy: function (expression) { return ViewChild.viewBy(_this_1, expression); },
                viewByName: function (componentName) { return ViewChild.viewByName(_this_1, componentName); },
                viewById: function (componentId) { return ViewChild.viewById(_this_1, componentId); },
            };
