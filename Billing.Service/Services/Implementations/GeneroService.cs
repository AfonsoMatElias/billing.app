...
namespace Billing.Service.Services.Implementations
{
	public class GeneroService : BaseService<Genero>, IGeneroService
	{
		...
		public async Task<GeneroDto> Find(Expression<Func<Genero, bool>> predicate, Func<IQueryable<Genero>, IQueryable<Genero>> queryable = null)
		{
			...
			return mapper.Map<GeneroDto>(dbModel);
		}

		public async Task<List<GeneroDto>> FindAll(Func<IQueryable<Genero>, IQueryable<Genero>> queryable = null)
		{
			// If the queryable argument is null define the default one
			if (queryable == null)
				queryable = func => func;
			...
			return mapper.Map<List<GeneroDto>>(dbModels);
		}

		public async Task<Pagination<GeneroDto>> FindAll(PageRange range, Func<IQueryable<Genero>, IQueryable<Genero>> queryable = null)
		{
			if (range == null)
				return new Pagination<GeneroDto>
				{
			...
			return new Pagination<GeneroDto>
			{
				...
				Data = mapper.Map<List<GeneroDto>>(pagination.Data)
			};
		}

		public async Task<GeneroDto> FindById(string uid, Func<IQueryable<Genero>, IQueryable<Genero>> queryable = null)
		{
			// If the queryable argument is null define the default one
			if (queryable == null)
				queryable = func => func;
		...
		public async Task Save(GeneroDto model, bool isCommit = true)
		{
			var dbModel = mapper.Map<Genero>(model);
		...
		public async Task Update(string uid, GeneroDto model, bool isCommit = true)
		{
			var _uid = uid.FromUID();
			if (_uid == null)
				throw new AppException("Identificador Inválido!", true);
			...
			// DB Model Update
			dbModel.UpdateFrom(mapper.Map<Genero>(model), new[] {
				"id"
			});
