...
namespace Billing.App.Controllers.Api
{
    ...
    public class SubCategoriaController : ControllerBase
    {
        ...
        public async Task<Response> Get([FromQuery] PageableQueryParam pageableQuery)
        {
            try
            {
                var dbData = await service.FindAll(Pagination.Of(pageableQuery.Page, pageableQuery.Size), 
                    queryable => queryable.Include(x => x.Categoria));
        ...
        // GET: api/SubCategoria/5:12837918237
        [HttpGet("{uid}")]
        ...
        [HttpPost]
        public async Task<Response> Post([FromBody] SubCategoriaDto model)
        {
            try
            {
                if (model == null)
        ...
        // PUT: api/SubCategoria/5:12837918237
        [HttpPut("{uid}")]
        public async Task<Response> Put(string uid, [FromBody] SubCategoriaDto model)
        {
            try
            {
                if (model == null)
        ...
        // DELETE: api/SubCategoria/5:12837918237
        [HttpDelete("{uid}")]
